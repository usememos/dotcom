...
[Getting Things Done](https://gettingthingsdone.com/what-is-gtd/) is most reliable when capture is fast, next actions are visible, and every project has a clear place to resume. Memos gives that practice a lightweight, self-hosted foundation: quick notes for capture, Markdown checkboxes for actions, tags for project context, Shortcuts for reusable views, and a searchable timeline that keeps the history.

This guide keeps the setup focused: one memo per commitment, one state tag for what happens next, and one project tag for related actions. Shortcuts keep next actions, waiting items, and active projects visible; checkboxes and Archive close the loop without discarding the record.
