...
5. The invited user opens **Inbox** and chooses **Accept** or **Decline** on the invitation. Invitations are also available in **Settings → Spaces**. After accepting, choose **Open space** or select it from the sidebar switcher.
...
Space admins can change the name, description, and icon from **Settings → Spaces**. Use the icon picker to choose an icon or emoji so the Space is easier to recognize in the sidebar and invitations.
...
Active instance administrators can also read and manage individual private and Space-only memos without Space membership. Collection listings still filter by audience.
...
### Move an existing memo

1. Open the memo's action menu and choose **Move to Space**.
2. Choose a destination Space or **Unassigned**.
3. Review **Visibility**. Moving a members-only memo to another Space changes which members can read it. Choosing Unassigned resets `SPACE` visibility to `PRIVATE`; you can select another available visibility before confirming.
4. Choose **Move**.

Existing reactions stay with the memo. Comments and linked memos keep their own placement and access rules; moving one memo does not move a whole discussion.
...
Select a Space from the sidebar switcher to scope Home, Explore, Calendar, Map, new memo creation, statistics, filters, and attachments to that Space. Choose the main **Memos** context to browse across the memos available to you. [Calendar](/docs/usage/memos#calendar) and [Map](/docs/usage/memos#map) show your own memos within that scope.

Opening a memo and returning to its collection preserves the Space and filters, so you can continue browsing where you left off.
...
Apart from active instance administrators, only a memo's author can edit its content or visibility and manage its attachments, references, and share links. For an assigned memo, these actions also require active membership. Reactions belong to the user who added them. Commenting or reacting to an assigned memo requires both read access and membership in its Space.

Active instance admins can read and manage individual memos without authorship or Space membership, but are not automatically Space members. Space admins govern the Space, but also cannot edit another author's memo. A Space must retain at least one active admin: promote another member before the last admin leaves, is removed, or is archived.
...
- [Export & Import](/docs/usage/export-import) — how Space placement is handled when moving your memos between instances
