...
On a fresh instance, this command initializes private access: ordinary memo pages require sign-in and anonymous Explore is unavailable. In 0.31, the saved `ACCESS` instance setting controls this policy independently of the canonical URL. The `stable` image follows stable releases; to pin this release explicitly, use `neosmemo/memos:0.31.0`.
