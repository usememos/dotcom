...
const CONTAINER_STYLES = {
  ...
  memo: "flex w-full max-w-[400px] flex-col bg-transparent p-0",
} as const;
...
const FALLBACK_STYLES = {
  ...
  memo: "inline-flex w-fit items-center gap-1 text-[10px] leading-4 text-[var(--mock-accent)] underline underline-offset-2 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--mock-accent)]",
} as const;
...
export function CarbonAdCard({ variant = "default" }: CarbonAdCardProps) {
  ...
  return (
    <div role="complementary" aria-label="Sponsored content" className={CONTAINER_STYLES[variant]} data-carbon-ad-card="">
      {ad ? <CarbonAdContent ad={ad} variant={variant} /> : <FallbackContent variant={variant} />}
    </div>
  );
}
...
function CarbonAdContent({ ad, variant }: { ad: CarbonAd; variant: keyof typeof CONTAINER_STYLES }) {
  ...
  return (
    <div
      ...
      className={
        variant === "memo"
          ? `grid w-full grid-rows-[auto_1fr] items-start gap-x-3 gap-y-1 ${ad.imageSrc ? "grid-cols-[130px_minmax(0,1fr)]" : "grid-cols-1"}`
          : "flex w-full flex-1 flex-col gap-1.5"
      }
      data-carbon-ad=""
    >
      <div className={variant === "memo" ? "contents" : "flex flex-1 items-start gap-3"}>
        {ad.imageSrc && (
          <a href={ad.href} target="_blank" rel="noopener sponsored" className={variant === "memo" ? "row-span-2" : "shrink-0"}>
        ...
        <a
          ...
          className={
            variant === "memo"
              ? "text-xs leading-[18px] text-[var(--mock-ink)] hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--mock-accent)]"
              : "text-xs leading-5 text-zinc-700 transition-colors hover:text-zinc-950 dark:text-zinc-300 dark:hover:text-zinc-50"
          }
        >
          {ad.description}
        </a>
      ...
      <a
        ...
        className={
          variant === "memo"
            ? `${ad.imageSrc ? "col-start-2" : ""} self-start justify-self-start text-[9px] leading-4 text-[var(--mock-muted)] hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--mock-accent)]`
            : "self-end text-[0.6875rem] text-zinc-400 uppercase tracking-wide transition-colors hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300"
        }
      >
        ads via Carbon
      </a>
...
function FallbackContent({ variant }: { variant: keyof typeof CONTAINER_STYLES }) {
  if (variant === "memo") {
    return (
      <a href={SPONSOR_URL} target="_blank" rel="noopener noreferrer" className={FALLBACK_STYLES.memo}>
        Sponsor Memos
