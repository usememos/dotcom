...
export function CarbonAdCard({ variant = "default" }: CarbonAdCardProps) {
  ...
  useEffect(() => {
    ...
    return carbonAdRuntime.register(mount, pathname);
  }, [pathname]);
  ...
  return (
    <div
      ...
      className={cn("group min-h-24", CONTAINER_STYLES[variant])}
      data-carbon-ad-card=""
    >
      <div ref={mountRef} className="w-full" data-carbon-ad-mount="" />
      <div className="flex flex-1 group-has-[#carbonads]:hidden" data-carbon-fallback="">
        <FallbackContent variant={variant} />
      </div>
    </div>
  );
}
