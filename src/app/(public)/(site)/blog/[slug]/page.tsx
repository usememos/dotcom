...
export default async function BlogPostPage({ params }: BlogPageProps) {
  ...
  return (
    <main className="flex flex-1 flex-col bg-white dark:bg-zinc-950">
      ...
      <section className="pb-8 pt-8 sm:pt-12 lg:pb-10">
                ...
                <div className="space-y-3 text-sm leading-7 text-zinc-600 dark:text-zinc-300">
                  ...
                  {data.tags && data.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-1">
                      {data.tags.map((tag) => (
                        <span key={tag} className="text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-500 dark:text-zinc-400">
                          {tag}
                        </span>
                      ))}
                    </div>
