  ...
  return (
    <>
      <div className="w-full mx-auto py-2 sm:px-4 flex flex-col justify-start items-start">
        <h1 className="w-full text-2xl text-start sm:text-4xl font-medium sm:font-bold mt-4">Blogs</h1>
        <h2 className="text-lg mt-4">Get the latest articles from Memos</h2>
        <div className="mt-6 w-full flex flex-col justify-start items-start gap-4 sm:gap-6">
          {frontmatters.map((frontmatter) => {
            const author = authorList.find((author) => author.name === frontmatter.author) as Author;

            return (
              <React.Fragment key={frontmatter.slug}>
                <div className="w-full bg-white p-4 border rounded-xl flex flex-row justify-start items-start gap-x-4">
                  ...
                  {frontmatter.feature_image && (
                    <Link
                      ...
                      href={`/blog/${frontmatter.slug}`}
                    >
                      <img className="w-72" src={frontmatter.feature_image} alt="" />
                    </Link>
                  )}
                </div>
              </React.Fragment>
